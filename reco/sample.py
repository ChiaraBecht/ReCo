...
class Sample:
    ...
    def __init__(self, name=None, vector_file=None, expected_reads=0, output_dir=None):
        ...
        self.logger.info(
            "ReCo v%s: initializing sample '%s'", get_versions()["version"], self.name
        )
        ...
        self.vector = None
        if self.vector_file:
            self.vector = Vector.from_file(
                name=Path(self.vector_file).stem,
                logger=self.logger,
        ...
        self.top_failed_file = os.path.join(
            self.output_dir, f"{self.name}_top{Config.TOP_FAILED_SEQUENCES}_failed.csv"
        )
    ...
    def expected_reads(self, new_expected_reads):
        ...
        if new_expected_reads == "" or isinstance(
            new_expected_reads, (int, numpy.int64)
        ):
    ...
    def create_dirs(self):
        ...
        try:
            ...
            raise FileExistsError(
                f"Output dir '{self.output_dir}' already exists!"
            ) from exc
...
class SingleSample(Sample):
    ...
    def __init__(self, sample_dict=None, output_dir=None):
        super().__init__(
            name=sample_dict["Sample name"],
            vector_file=sample_dict["Vector"],
            expected_reads=sample_dict["Expected reads"],
        ...
        self.cutadapt_output_file = os.path.join(
            self.output_dir, Path(self.fastq_file).stem + ".cutadapt_out"
        )
        self.final_trimmed_file = os.path.join(
            Path(self.cutadapt_output_file).parent,
            Path(self.cutadapt_output_file).stem + ".final_cutadapt_out",
        )
        self.sam_file = os.path.join(
            Path(self.final_trimmed_file).parent,
            Path(self.final_trimmed_file).stem + ".sam",
        )
        ...
        self.readcount_file = os.path.join(
            self.output_dir, f"{self.name}_final_guidecounts.csv"
        )
    ...
    def run(self, remove_unused_files=True, cores=1):
        ...
        self.cassette_information = CassetteFinder.run(
            logger=self.logger,
            library=self.lib,
        ...
        if (
            self.cassette_information["lib_dir"] is None
            and self.cassette_information["homology"] is None
        ):
        ...
        print(
            "  Cassette determination in:",
            datetime.datetime.now() - cassette_information_time,
        )

        template_information_time = datetime.datetime.now()
        yield "Determine 3Cs template sequence ..."
        if self.vector:
            self.template_information = self.vector.find_template(
                template_length=self.lib.sequence_length,
                prefix=self.cassette_information["homology"],
            )
            self.lib.add_guide(
                guide_name=f"template-{self.template_information['template_name']}",
                sequence=self.template_information["template_sequence"],
            )
        ...
        print(
            "  Template determination in:",
            datetime.datetime.now() - template_information_time,
        )

        trimming_time = datetime.datetime.now()
        yield "Trim ..."
        sample_trimmer = CutadaptTrimmer(
            logger=self.logger,
            max_n=Config.CUTADAPT_MAX_N,
            error_tolerance=Config.CUTADAPT_ERROR_TOLERANCE,
        )

        cutadapt_output = sample_trimmer.run_single(
            sequence_length=self.lib.sequence_length,
            homology=self.cassette_information["homology"],
            output_file=self.cutadapt_output_file,
            fastq_file=self.fastq_file,
            cores=cores,
        )
        self.get_trimming_rate(cutadapt_output=cutadapt_output, sample_type="single")
        sample_trimmer.finalize_single(
            cutadapt_output_file=self.cutadapt_output_file,
            finalized_output=self.final_trimmed_file,
            sequence_length=self.lib.sequence_length,
        )
        ...
        count_tables = rc_writer.counts_from_sam(
            sam_file=self.sam_file, library=self.lib
        )
        self.rc_table = rc_writer.write_counter(
            count_tables=count_tables,
            count_col_name=self.name,
            counts_file=self.readcount_file,
            failed_file=self.failed_file,
            top_failed_file=self.top_failed_file,
        )
    ...
    def write_report(self):
        ...
        with open(self.report_file, "w", encoding="utf-8") as writefile:
            ...
            writefile.write(
                f"Library direction: {self.cassette_information['lib_dir']}\n"
            )
            if self.template_information:
                writefile.write(
                    f"Template name: {self.template_information['template_name']}\n"
                )
                writefile.write(
                    f"Template sequence: {self.template_information['template_sequence']}\n"
                )
            else:
            ...
            writefile.write(
                f"Trimmed reads: {self.trimmed_reads:n} ({round(self.trimmed_reads / (self.observed_reads / 100), 2)}% of observed)\n"
            )
            writefile.write(
                f"Aligned reads: {self.rc_table[self.name].shape[0]:n} ({round(self.rc_table[self.name].shape[0] / (self.observed_reads / 100), 2)}% of observed)\n"
            )
...
class PairedSample(Sample):
    ...
    def __init__(self, sample_dict=None, output_dir=None):
        ...
        self.cutadapt_output_file_1 = os.path.join(
            self.output_dir, Path(self.fastq_file_1).stem + ".cutadapt_out"
        )
        self.final_trimmed_file_1 = os.path.join(
            Path(self.cutadapt_output_file_1).parent,
            Path(self.cutadapt_output_file_1).stem + ".final_cutadapt_out",
        )
        self.sam_file_1 = os.path.join(
            Path(self.final_trimmed_file_1).parent,
            Path(self.final_trimmed_file_1).stem + ".sam",
        )

        self.cutadapt_output_file_2 = os.path.join(
            self.output_dir, Path(self.fastq_file_2).stem + ".cutadapt_out"
        )
        self.final_trimmed_file_2 = os.path.join(
            Path(self.cutadapt_output_file_2).parent,
            Path(self.cutadapt_output_file_2).stem + ".final_cutadapt_out",
        )
        self.sam_file_2 = os.path.join(
            Path(self.final_trimmed_file_2).parent,
            Path(self.final_trimmed_file_2).stem + ".sam",
        )
        ...
        self.readcount_file = os.path.join(
            self.output_dir, f"{self.name}_combinations_final_guidecounts.csv"
        )

        self.readcount_file_read_1 = os.path.join(
            self.output_dir, f"{self.name}_R1_final_guidecounts.csv"
        )
        self.failed_file_read_1 = os.path.join(
            self.output_dir, f"{self.name}_R1_failed.csv"
        )
        self.top_failed_file_read_1 = os.path.join(
            self.output_dir,
            f"{self.name}_top{Config.TOP_FAILED_SEQUENCES}_R1_failed.csv",
        )

        self.readcount_file_read_2 = os.path.join(
            self.output_dir, f"{self.name}_R2_final_guidecounts.csv"
        )
        self.failed_file_read_2 = os.path.join(
            self.output_dir, f"{self.name}_R2_failed.csv"
        )
        self.top_failed_file_read_2 = os.path.join(
            self.output_dir,
            f"{self.name}_top{Config.TOP_FAILED_SEQUENCES}_R2_failed.csv",
        )
    ...
    def info(self):
        ...
        return {
            ...
            "fastq_file_2": self.fastq_file_2,
            "lib_file_1": self.lib_file_1,
            "lib_file_2": self.lib_file_2,
        }
    ...
    def run(self, remove_unused_files=True, cores=1):
        ...
        self.cassette_information_1 = CassetteFinder.run(
            logger=self.logger,
            library=self.lib_1,
            fastq_file=self.fastq_file_1,
            h5_length=Config.HOMOLOGY_5_LENGTH,
            h3_length=Config.HOMOLOGY_3_LENGTH,
            guides_to_test_percent=Config.GUIDES_TO_TEST_PERCENT,
            reads_to_test=Config.READS_TO_TEST,
        )
        self.cassette_information_2 = CassetteFinder.run(
            logger=self.logger,
            library=self.lib_2,
            fastq_file=self.fastq_file_2,
            h5_length=Config.HOMOLOGY_5_LENGTH,
            h3_length=Config.HOMOLOGY_3_LENGTH,
            guides_to_test_percent=Config.GUIDES_TO_TEST_PERCENT,
            reads_to_test=Config.READS_TO_TEST,
        ...
        if self.vector:
            ...
            self.lib_1.add_guide(
                guide_name=f"template-{self.template_information_1['template_name']}",
                sequence=self.template_information_1["template_sequence"],
            )
            ...
            self.lib_2.add_guide(
                guide_name=f"template-{self.template_information_2['template_name']}",
                sequence=self.template_information_2["template_sequence"],
            )
        ...
        cutadapt_output_file_1 = os.path.join(
            self.output_dir, Path(self.fastq_file_1).stem + ".cutadapt_out"
        )
        cutadapt_output_file_2 = os.path.join(
            self.output_dir, Path(self.fastq_file_2).stem + ".cutadapt_out"
        )
        cutadapt_output = sample_trimmer.run_paired(
        ...
        combinations = sample_trimmer.finalize_paired(
            cutadapt_output_file_1=self.cutadapt_output_file_1,
            cutadapt_output_file_2=self.cutadapt_output_file_2,
            finalized_output_1=self.final_trimmed_file_1,
            finalized_output_2=self.final_trimmed_file_2,
            sequence_length_1=self.lib_1.sequence_length,
            sequence_length_2=self.lib_2.sequence_length,
        )

        yield "Align ..."
        aligner_1 = Bowtie2Aligner(
            logger=self.logger,
            seed_length=Config.BOWTIE2_SEED_LENGTH,
            seed_mismatch=Config.BOWTIE2_SEED_MISMATCH,
            interval_function=Config.BOWTIE2_INTERVAL_FUNCTION,
            max_mismatch=Config.BOWTIE2_MAX_MISMATCH,
            seq_length=self.lib_1.sequence_length,
            bowtie2_index=self.lib_1_bowtie2_index,
            final_trimmed_file=self.final_trimmed_file_1,
            sam_file=self.sam_file_1,
        )
        aligner_1.run(cores=cores)

        aligner_2 = Bowtie2Aligner(
            logger=self.logger,
            seed_length=Config.BOWTIE2_SEED_LENGTH,
            seed_mismatch=Config.BOWTIE2_SEED_MISMATCH,
            interval_function=Config.BOWTIE2_INTERVAL_FUNCTION,
            max_mismatch=Config.BOWTIE2_MAX_MISMATCH,
            seq_length=self.lib_2.sequence_length,
            bowtie2_index=self.lib_2_bowtie2_index,
            final_trimmed_file=self.final_trimmed_file_2,
            sam_file=self.sam_file_2,
        )
        aligner_2.run(cores=cores)

        yield "Collect read counts ..."
        rc_writer = ReadcountWriter(logger=self.logger)
        count_tables_1 = rc_writer.counts_from_sam(
            sam_file=self.sam_file_1, library=self.lib_1
        )
        count_tables_2 = rc_writer.counts_from_sam(
            sam_file=self.sam_file_2, library=self.lib_2
        )

        self.rc_table_1 = rc_writer.write_counter(
            count_tables=count_tables_1,
            count_col_name=self.name + "_R1",
            counts_file=self.readcount_file_read_1,
            failed_file=self.failed_file_read_1,
            top_failed_file=self.top_failed_file_read_1,
        )
        self.rc_table_2 = rc_writer.write_counter(
            count_tables=count_tables_2,
            count_col_name=self.name + "_R2",
            counts_file=self.readcount_file_read_2,
            failed_file=self.failed_file_read_2,
            top_failed_file=self.top_failed_file_read_2,
        )

        self.aligned_combis = self.write_combinations(
            combinations,
            count_tables_1["sequence_hitlist"],
            count_tables_2["sequence_hitlist"],
        )

        read_count_df = pd.read_csv(
            self.readcount_file, index_col=["Guide_1", "Guide_2"]
        )
        end_time = datetime.datetime.now()

        print(" ===========> Running time: ", end_time - start_time)
        yield "Generate plots ..."
        p_panel_combis = PlotPanel(
            logger=self.logger,
            sample_name=self.name,
            sample_type="paired",
            ...
            observed_reads=self.observed_reads,
            trimmed_reads=self.trimmed_reads,
            aligned_reads=read_count_df[self.name].sum(),
            template_marker="template",
            output_dir=self.output_dir,
        )
        p_panel_combis.generate()

        p_panel_read_1 = PlotPanel(
            logger=self.logger,
            sample_name=self.name + "_R1",
            sample_type="single",
            read_count_df=self.rc_table_1,
            expected_reads=self.expected_reads,
            observed_reads=self.observed_reads,
            trimmed_reads=self.trimmed_reads,
            aligned_reads=self.rc_table_1[self.name + "_R1"].sum(),
            template_marker="template",
            output_dir=self.output_dir,
        )
        p_panel_read_1.generate()

        p_panel_read_2 = PlotPanel(
            logger=self.logger,
            sample_name=self.name + "_R2",
            sample_type="single",
            read_count_df=self.rc_table_2,
            expected_reads=self.expected_reads,
            observed_reads=self.observed_reads,
            trimmed_reads=self.trimmed_reads,
            aligned_reads=self.rc_table_2[self.name + "_R2"].sum(),
            template_marker="template",
            output_dir=self.output_dir,
        )
        ...
        zip_file_list(
            file_list=[
                ...
                p_panel_combis.pdf_file,
                p_panel_combis.png_file,
                p_panel_read_1.pdf_file,
                p_panel_read_1.png_file,
                p_panel_read_2.pdf_file,
                p_panel_read_2.png_file,
                self.report_file,
            ],
            output_file=self.zip_file,
        )
    ...
    def write_combinations(self, combinations, count_table_1, count_table_2):
        ...
        with open(self.failed_file, "w", encoding="utf-8") as failed_file_writer, open(
            self.top_failed_file, "w", encoding="utf-8"
        ) as top_failed_file_writer:
            failed_file_writer.write("Sequence_1,Sequence_2,Guide_1,Guide_2,count\n")
            top_failed_file_writer.write(
                "Sequence_1,Sequence_2,Guide_1,Guide_2,count\n"
            )
            combi_counter = 0
            for combi, count in failed_counts.most_common():
                failed_file_writer.write(
                    f"{combi[0]},{combi[1]},{combi[2]},{combi[3]},{count}\n"
                )
    ...
    def write_report(self):
        ...
        with open(self.report_file, "w", encoding="utf-8") as writefile:
            ...
            writefile.write(f"{self}\n")
            writefile.write(
                f"Library 1: {self.lib_file_1} ({len(self.lib_1)} guides)\n"
            )
            writefile.write(
                f"Library 2: {self.lib_file_2} ({len(self.lib_2)} guides)\n"
            )
            writefile.write(f"Vector: {self.vector_file}\n")
            writefile.write(f"FastQ read 1: {self.fastq_file_1}\n")
            writefile.write(f"FastQ read 2: {self.fastq_file_2}\n")
            writefile.write(
                f"Homology read 1: {self.cassette_information_1['homology']}\n"
            )
            writefile.write(
                f"Homology read 2: {self.cassette_information_2['homology']}\n"
            )
            writefile.write(
                f"Library direction 1: {self.cassette_information_1['lib_dir']}\n"
            )
            writefile.write(
                f"Library direction 2: {self.cassette_information_2['lib_dir']}\n"
            )
            if self.template_information_1 and self.template_information_2:
                writefile.write(
                    f"Template name 1: {self.template_information_1['template_name']}\n"
                )
                writefile.write(
                    f"Template name 2: {self.template_information_2['template_name']}\n"
                )
                writefile.write(
                    f"Template sequence read 1: {self.template_information_1['template_sequence']}\n"
                )
                writefile.write(
                    f"Template sequence read 2: {self.template_information_2['template_sequence']}\n"
                )
            else:
                writefile.write("No template vector provided.")
            writefile.write(f"Expected combinations: {self.expected_reads:n}\n")
            writefile.write(
                f"Observed combinations: {self.observed_reads:n} ({round(self.observed_reads / (self.expected_reads / 100), 2)}% of expected)\n"
            )
            writefile.write(
                f"Trimmed combinations: {self.trimmed_reads:n} ({round(self.trimmed_reads / (self.observed_reads / 100), 2)}% of observed)\n"
            )
            writefile.write(
                f"Aligned combinations: {self.aligned_combis:n} ({round(self.aligned_combis / (self.observed_reads / 100), 2)}% of observed)\n"
            )
